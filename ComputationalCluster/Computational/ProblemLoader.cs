...
namespace ComputationalCluster.Client
{
    public static class ProblemLoader
    {
        ...
        {
            newExampleObject Empty = new newExampleObject();
              ...
              if (filename != null)
            {
                ...
                try
                {
                    ...
                    for (int i=0; i < demands.Count; i++)
                    {
                        ServiceDurationList[i] = durations[i];
                        ServiceEndList[i] = TimeAvailable[0];
                        ServiceBeginList[i] = TimeAvailable[i];
                        DeliverDemandList.Add(-Convert.ToDouble(demands[i], CultureInfo.InvariantCulture));
                    }

                    Empty = new newExampleObject(LocationList.ToArray(), DeliverDemandList.ToArray(), ServiceDurationList.ToArray(), 
